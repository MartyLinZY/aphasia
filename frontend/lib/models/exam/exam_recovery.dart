...
class ExamQuestionSet{
  ...
  ExamQuestionSet({String? id, this.name= "新测评", this.description = "", this.recovery = false})
      : published = false,
        _id = id;
  ...
  static Future<ExamQuestionSet?> getById({required String id}) async {
    try {
      ...
      await HttpClientManager().get(url: "${HttpConstants.backendBaseUrl}/api/exams/$id");
  ...
  static Future<List<ExamQuestionSet>> getByDoctorUserId({required String userId, bool getRecovery = false}) async {
    // List<dynamic> jsonArr;
    // TODO: uncomment these two line
    List<dynamic> jsonData;
    if (!getRecovery) {
      jsonData = await HttpClientManager()
          .get(url: "${HttpConstants.backendBaseUrl}/api/doctors/$userId/exams");
    } else {
      jsonData = await HttpClientManager()
          .get(url: "${HttpConstants.backendBaseUrl}/api/doctors/$userId/recoveries");
    }
  ...
  static Future<ExamQuestionSet> createExam({required String name, String description = "", bool isRecovery = false}) async {
    var exam = ExamQuestionSet(name: name, description: description, recovery: isRecovery);

    var jsonData = await HttpClientManager().post(url: "${HttpConstants.backendBaseUrl}/api/exams", body: jsonEncode(exam.toJson()));
  ...
  Future<void> updateName({required String newName}) async {
    await HttpClientManager().patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/name/$newName", body: '{}');

    name = newName;
  }

  Future<void> updateDescription({required String newDescription}) async {
    await HttpClientManager().patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/desc/$newDescription", body: '{}');

    description = newDescription;
  }

  /// 新增测评亚项，发送http请求更新后台数据库，若http请求失败，本地数据不变。要求测评未发布，否则抛出[EditPublishedQuestionSetException]异常
  Future<QuestionCategory> addCategory({String description = "新亚项", int? insertAt}) async {
    ...
    // 默认添加一条按子项得分求和得评分规则
    var newCategory = QuestionCategory(description: description)..rules.add(EvalBySubCategoryScoreSum());
    await HttpClientManager()
        .post(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/category", body: jsonEncode(newCategory.toJson()));
  ...
  Future<QuestionCategory> deleteCategory({required int categoryIndex}) async {
    ...
    await HttpClientManager()
        .delete(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex");
  ...
  Future<void> updateCategory({required QuestionCategory updatedCategory, required int categoryIndex}) async {
    _checkPublished();
    _checkCategoryIndex(categoryIndex);

    await HttpClientManager().patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex", body: jsonEncode(updatedCategory.toJson()));
  ...
  Future<void> moveCategoryUp({required int categoryIndex}) async {
    ...
    await HttpClientManager().patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/up", body: '{}');
  ...
  Future<void> moveCategoryDown({required int categoryIndex}) async {
    ...
    await HttpClientManager().patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/down", body: '{}');
  ...
  Future<QuestionSubCategory> addSubCategory({String description = "新子项", required int categoryIndex}) async {
    _checkPublished();
    _checkCategoryIndex(categoryIndex);
    QuestionCategory category = categories[categoryIndex];

    // 默认添加一条评分规则：下属所有题目得分之和
    var newSubCategory = QuestionSubCategory(description: description)..evalRules.add(EvalSubCategoryByQuestionScoreSum());
    await HttpClientManager()
        .post(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategory", body: jsonEncode(newSubCategory.toJson()));
  ...
  Future<void> updateSubCategory({required QuestionSubCategory updatedSubCategory, required int categoryIndex, required int subCategoryIndex}) async {
    _checkPublished();
    _checkSubCategoryIndex(categoryIndex, subCategoryIndex);
    QuestionCategory category = categories[categoryIndex];

    await HttpClientManager().patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex", body: jsonEncode(updatedSubCategory.toJson()));
  ...
  Future<QuestionSubCategory> deleteSubCategory({required int categoryIndex, required int subCategoryIndex}) async {
    ...
    await HttpClientManager().delete(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex");
  ...
  Future<void> moveSubCategoryUp({required int categoryIndex, required int subCategoryIndex}) async {
    ...
    await HttpClientManager().patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/up", body: '{}');

    if (subCategoryIndex > 0) {
      QuestionSubCategory tmp = categories[categoryIndex].subCategories[subCategoryIndex - 1];
      categories[categoryIndex].subCategories[subCategoryIndex - 1] = categories[categoryIndex].subCategories[subCategoryIndex];
  ...
  Future<void> moveSubCategoryDown({required int categoryIndex, required int subCategoryIndex}) async {
    ...
    await HttpClientManager().patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/down", body: '{}');
  ...
  _checkQuestionIndex(int categoryIndex, int subCateIndex, int questionIndex) {
    ...
    if (questionIndex < 0 || questionIndex >= categories[categoryIndex].subCategories[subCateIndex].questions.length) {
      throw RangeError.index(questionIndex, categories[categoryIndex].subCategories[subCateIndex].questions);
    }
  }

  /// 新增问题，发送http请求更新后台数据库，若http请求失败，本地数据不变。要求测评未发布，否则抛出[EditPublishedQuestionSetException]异常
  Future<Question> addQuestion(Question questionToAdd, {required int categoryIndex, required int subCategoryIndex}) async {
    _checkPublished();
    if (categoryIndex >= categories.length || subCategoryIndex >= categories[categoryIndex].subCategories.length) {
      if (categoryIndex >= categories.length) {
        throw RangeError.index(categoryIndex, categories);
      } else {
        throw RangeError.index(subCategoryIndex, categories[categoryIndex].subCategories);
      }
    }

    var jsonData = await HttpClientManager()
        .post(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/question",
    ...
    categories[categoryIndex].subCategories[subCategoryIndex].questions.add(newQuestion);

    return newQuestion;
  }

  Future<void> updateQuestion(Question updated, {required int categoryIndex, required int subCategoryIndex, required int questionIndex}) async {
    _checkPublished();
    _checkQuestionIndex(categoryIndex, subCategoryIndex, questionIndex);

    await HttpClientManager()
        .patch(url: "${HttpConstants.backendBaseUrl}/api/questions/${updated.id}",
        body: jsonEncode(updated.toJson()));

    categories[categoryIndex].subCategories[subCategoryIndex].questions[questionIndex] = updated;
  }
  ...
  Future<Question> deleteQuestion({required int categoryIndex, required int subCategoryIndex, required int questionIndex}) async {
    ...
    await HttpClientManager()
        .delete(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/questions/$questionIndex");

    return categories[categoryIndex].subCategories[subCategoryIndex].questions.removeAt(questionIndex);
  }
  ...
  async {
    ...
    await HttpClientManager()
        .patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/questions/$questionIndex/up", body: '{}');

    var subCategoryToUpdate = categories[categoryIndex].subCategories[subCategoryIndex];
    if (questionIndex > 0) {
      var tmp = subCategoryToUpdate.questions[questionIndex - 1];
      subCategoryToUpdate.questions[questionIndex - 1] = subCategoryToUpdate.questions[questionIndex];
  ...
  Future<void> moveQuestionDown({required int categoryIndex, required int subCategoryIndex, required int questionIndex}) async {
    ...
    await HttpClientManager()
        .patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/questions/$questionIndex/down", body: '{}');
  ...
  Future<void> publish() async {
    ...
    await HttpClientManager()
        .patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id", body: '{}');
  ...
  Future<void> addDiagnosisRule({required DiagnosisRule newRule, int? ruleIndex}) async {
    ...
    await HttpClientManager()
        .post(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/diagnosisRule", body: jsonEncode(newRule.toJson()));
  ...
  Future<DiagnosisRule> deleteDiagnosisRule({required int ruleIndex}) async {
    ...
    await HttpClientManager()
        .delete(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/diagnosisRules/$ruleIndex");
  ...
  Future<void> updateDiagnosisRule({required DiagnosisRule updatedRule, required int ruleIndex}) async {
    ...
    await HttpClientManager()
        .patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/diagnosisRules/$ruleIndex", body: jsonEncode(updatedRule.toJson()));

    diagnosisRules[ruleIndex] = updatedRule;
  }

  _checkCategoryEvalRuleIndex({required int categoryIndex, required int ruleIndex}) {
    _checkCategoryIndex(categoryIndex);
    var category = categories[categoryIndex];
  ...
  Future<void> addCategoryEvalRule({required int categoryIndex, int? ruleIndex, required ExamCategoryEvalRule newRule}) async {
    ...
    if (ruleIndex != categories[categoryIndex].rules.length) {
      _checkCategoryEvalRuleIndex(categoryIndex: categoryIndex, ruleIndex: ruleIndex);
    }
  ...
  _checkSubCategoryEvalRuleIndex({required int categoryIndex, required int subCategoryIndex, required int ruleIndex}) {
    _checkSubCategoryIndex(categoryIndex, subCategoryIndex);
  ...
  Future<void> addSubCategoryEvalRule({required int categoryIndex, required int subCategoryIndex, int? ruleIndex, required ExamSubCategoryEvalRule newRule}) async {
    ...
    QuestionSubCategory subCategory = categories[categoryIndex].subCategories[subCategoryIndex];
    ruleIndex ??= subCategory.evalRules.length;

    if (ruleIndex != subCategory.evalRules.length) {
      _checkSubCategoryEvalRuleIndex(categoryIndex: categoryIndex, subCategoryIndex: subCategoryIndex, ruleIndex: ruleIndex);
    }
  ...
  _checkSubCategoryTerminateRuleIndex({required int categoryIndex, required int subCategoryIndex, required int ruleIndex}) {
    _checkSubCategoryIndex(categoryIndex, subCategoryIndex);
    var subCategory = categories[categoryIndex].subCategories[subCategoryIndex];
  ...
  Future<void> addSubCategoryTerminateRule({required int categoryIndex, required int subCategoryIndex, int? ruleIndex, required TerminateRule newRule}) async {
    ...
    if (ruleIndex != subCategory.terminateRules.length) {
      _checkSubCategoryTerminateRuleIndex(categoryIndex: categoryIndex, subCategoryIndex: subCategoryIndex, ruleIndex: ruleIndex);
    }

    await HttpClientManager()
        .post(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/terminateRule", body: jsonEncode(newRule.toJson()));
  ...
  Future<void> updateSubCategoryTerminateRule({required int categoryIndex, required int subCategoryIndex, required int ruleIndex, required TerminateRule updatedEvalRule}) async {
    ...
    await HttpClientManager()
        .patch(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/terminateRules/$ruleIndex", body: jsonEncode(updatedEvalRule.toJson()));

    subCategory.terminateRules[ruleIndex] = updatedEvalRule;
  }

  Future<TerminateRule> deleteSubCategoryTerminateRule({required int categoryIndex, required int subCategoryIndex, required int ruleIndex}) async {
    _checkPublished();
    _checkSubCategoryTerminateRuleIndex(categoryIndex: categoryIndex, subCategoryIndex: subCategoryIndex, ruleIndex: ruleIndex);
    QuestionSubCategory subCategory = categories[categoryIndex].subCategories[subCategoryIndex];

    await HttpClientManager()
        .delete(url: "${HttpConstants.backendBaseUrl}/api/exams/$_id/categories/$categoryIndex/subCategories/$subCategoryIndex/terminateRules/$ruleIndex");
  ...
  void checkSettingBeforePublish() {
      ...
      _passOrThrowIncompleteException(needSubCategory, needSubCateEvalRule, needQuestion, needCateEvalRule, needEvalRule, needDiagnosisRule, needCategory);
      for (int i = 0;i < categories.length;i++) {
        var category = categories[i];
        needSubCategory = category.subCategories.isEmpty;
        needCateEvalRule = category.rules.isEmpty;
        _passOrThrowIncompleteException(needSubCategory, needSubCateEvalRule, needQuestion, needCateEvalRule, needEvalRule, needDiagnosisRule, needCategory, cateIndex: i);
        for (var j = 0;j < category.subCategories.length;j++) {
          ...
          _passOrThrowIncompleteException(needSubCategory, needSubCateEvalRule, needQuestion, needCateEvalRule, needEvalRule, needDiagnosisRule, needCategory, cateIndex: i, subCateIndex: j);
        }
      }
  }

  void _passOrThrowIncompleteException(bool needSubCategory,
    ...
    bool needCategory, {
    int? cateIndex, int? subCateIndex,
  }) {
    if (needSubCategory ||needSubCateEvalRule ||needQuestion || needCateEvalRule || needEvalRule || needDiagnosisRule || needCategory) {
