...
1. **不做类型分类**。Broca/Wernicke/Anomic… 由 3 个维度定义（流畅度、**听理解**、**复述**），后两者**不在自发对话里**，光靠特征拿不到。完整分型需新增结构化子任务（理解/复述/命名，可做成"套题"），把分数喂进 WAB 决策树。从对话特征出发最多可靠粗分 3~4 类（Broca/Anomic/Wernicke）。
