...
public class PythonInvoker {
    ...
    // 与 application.properties llm.timeout-seconds 保持一致
    ...
    public static Map<String, Object> invokeDiagnose(String mode, String conversation) throws Exception {
        ...
        if (!finished) {
            ...
            throw new BusinessErrorException("LLM 诊断服务调用超时");
        }
        if (process.exitValue() != 0) {
            throw new BusinessErrorException("LLM 诊断服务调用失败（exit code: " + process.exitValue() + "）");
        }
    ...
    public static Map<String, Object> invokeRepair(String conversation) throws Exception {
        ...
        if (!finished) {
            process.destroyForcibly();
            throw new BusinessErrorException("LLM 修复服务调用超时");
        }
        if (process.exitValue() != 0) {
            throw new BusinessErrorException("LLM 修复服务调用失败（exit code: " + process.exitValue() + "）");
        }
